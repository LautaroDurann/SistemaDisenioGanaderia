...
PARCELAS = [
    ('Parcela Norte', 500, 400, Parcela.ESTADO_EN_PASTOREO),
    ('Parcela Sud', 600, 350, Parcela.ESTADO_EN_PASTOREO),
    ('Parcela Este', 450, 420, Parcela.ESTADO_EN_DESCANSO),
    ('Parcela Oeste', 550, 380, Parcela.ESTADO_EN_PASTOREO),
    ('Corral', 120, 90, Parcela.ESTADO_EN_MANTENIMIENTO),
]

KG_PRECIO = {'Toro': '3200', 'Vaca': '2800', 'Ternero': '3500'}

# (caravana, nombre, raza, color, nacimiento, peso_actual, parcela)
TOROS = [
    (2500100101, 'Rey', 'Angus', 'Negro', date(2019, 3, 12), 920, 'Parcela Norte'),
    (2500100102, 'Don Juan', 'Braford', 'Colorado', date(2020, 5, 2), 880, 'Parcela Norte'),
    (2500100103, 'Pampa', 'Hereford', 'Colorado', date(2019, 11, 20), 850, 'Parcela Norte'),
    (2500100104, 'Galeno', 'Brangus', 'Barcino', date(2021, 2, 14), 800, 'Parcela Norte'),
]

# (caravana, nombre, raza, color, nacimiento, peso_actual, parcela)
VACAS = [
    (2500100201, 'Paloma', 'Braford', 'Colorado', date(2020, 9, 15), 520, 'Parcela Norte'),
    (2500100202, 'Luna', 'Angus', 'Negro', date(2021, 4, 3), 510, 'Parcela Sud'),
    (2500100203, 'Mora', 'Braford', 'Overo', date(2019, 8, 22), 540, 'Parcela Norte'),
    (2500100204, 'Nina', 'Hereford', 'Colorado', date(2022, 1, 18), 490, 'Parcela Sud'),
    (2500100205, 'Aurora', 'Angus', 'Negro', date(2020, 11, 30), 530, 'Parcela Sud'),
    (2500100206, 'Canela', 'Braford', 'Colorado', date(2021, 7, 11), 505, 'Parcela Este'),
    (2500100207, 'Dulce', 'Brangus', 'Barcino', date(2019, 5, 25), 560, 'Parcela Oeste'),
    (2500100208, 'Estrella', 'Hereford', 'Colorado', date(2022, 3, 7), 480, 'Parcela Este'),
    (2500100209, 'Flora', 'Angus', 'Negro', date(2021, 10, 19), 515, 'Parcela Este'),
    (2500100210, 'Gringa', 'Braford', 'Colorado', date(2020, 2, 10), 545, 'Parcela Sud'),
    (2500100211, 'Jacinta', 'Brangus', 'Barcino', date(2022, 6, 28), 470, 'Parcela Este'),
    (2500100212, 'Lucía', 'Hereford', 'Colorado', date(2019, 12, 5), 555, 'Parcela Sud'),
    (2500100213, 'Mancha', 'Braford', 'Overo', date(2021, 1, 29), 525, 'Parcela Oeste'),
    (2500100214, 'Morena', 'Angus', 'Negro', date(2023, 4, 21), 460, 'Parcela Oeste'),
    (2500100215, 'Negra', 'Angus', 'Negro', date(2022, 8, 14), 485, 'Parcela Norte'),
    (2500100216, 'Ñata', 'Braford', 'Colorado', date(2020, 6, 17), 535, 'Parcela Oeste'),
    (2500100217, 'Perla', 'Hereford', 'Colorado', date(2021, 12, 9), 495, 'Parcela Este'),
    (2500100218, 'Rosa', 'Brangus', 'Colorado', date(2023, 2, 26), 450, 'Parcela Este'),
    (2500100219, 'Sombra', 'Angus', 'Negro', date(2022, 10, 5), 475, 'Parcela Oeste'),
    (2500100220, 'Violeta', 'Braford', 'Colorado', date(2020, 3, 31), 550, 'Parcela Oeste'),
]
...
class Command(BaseCommand):
    ...
    def _parcela(self, establecimiento, nombre):
    ...
    def _crear_animal(self, caravana, nombre, sexo, raza, color, nacimiento, peso_actual, categoria, parcela, madre=None, padre=None, costo=None):
        ...
        animal, creado = Animal.objects.get_or_create(
            ...
            defaults={
                ...
                'padre': padre,
                'parcela': parcela,
                'establecimiento': parcela.establecimiento,
            },
        )
    ...
    def handle(self, *args, **options):
        ...
        parcelas = {}
        for nombre, ancho, largo, estado in PARCELAS:
            parcela, _ = Parcela.objects.get_or_create(
                establecimiento=establecimiento,
                descripcion=nombre,
        ...
        for caravana, nombre, raza, color, nacimiento, peso, parcela in TOROS:
            animal, creado = self._crear_animal(
                caravana, nombre, 'Macho', raza, color, nacimiento, peso,
                'Toro', parcelas[parcela], costo=_peso('2400000.00'),
            )
        ...
        for caravana, nombre, raza, color, nacimiento, peso, parcela in VACAS:
            animal, creado = self._crear_animal(
                ...
                'Vaca', parcelas[parcela], costo=_peso('1100000.00'),
            )
        ...
        for i, (caravana, nombre, sexo, raza, color, nacimiento, peso, madre_caravana) in enumerate(TERNEROS):
            ...
            parcela = madre.parcela
            animal, creado = self._crear_animal(
                caravana, nombre, sexo, raza, color, nacimiento, peso,
                'Ternero', parcela, madre=madre, padre=padre, costo=None,
            )
            if creado:
                creados += 1
